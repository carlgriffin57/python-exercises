...
fruits_with_more_than_two_vowels = [fruit for fruit in fruits if sum([1 for vowel in 'aeiou' if vowel in fruit.lower()]) > 2]

# Exercise 4 - make a variable named fruits_with_only_two_vowels. The result should be 
# ['mango', 'kiwi', 'strawberry']
fruits_with_only_two_vowels = [fruit for fruit in fruits if sum([1 for vowel in 'aeiou' if vowel in fruit.lower()]) == 2]
...
# Exercise 14 - use a list comprehension w/ a conditional in order to produce a list of numbers with 2 or 
# more numerals
print([number for number in numbers if len(str(abs(number))) >= 2])
...
divs = [2,3,5,7]
primes = [number for number in set(numbers) if 0 not in {1 if ((number%i != 0) | (number in divs)) & (number > 0) else 0 for i in divs}]
